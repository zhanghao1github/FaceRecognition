...
for i in range(lent):
    ...
    if v.endswith(".jpg"):  # 只处理.jpg为扩展名的文件
        ...
        img1 = resizeImage(img, 48, 48)

        img2 = Image.new("RGB", (48, 48), color="white")

        w1, h1 = img1.size

        img2 = Image.new("RGB", (48, 48), color="white")

        img2.paste(img1, box=(int((48 - w1) / 2), int((48 - h1) / 2)))
...
xData = np.reshape(xData, (-1, 48, 48, 3))
...
if (ifRestartT is False) and os.path.exists(trainResultPath + ".h5"):
    ...
    model.add(k.layers.Conv2D(filters=4, kernel_size=(3, 3), input_shape=(48, 48, 3), data_format="channels_last", activation="relu"))
